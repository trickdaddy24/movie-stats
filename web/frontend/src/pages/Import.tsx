...
function RadarrTab({ progress, connect, reset, cancel }: ReturnType<typeof useImportProgress>) {
  // Auto-load saved credentials from .env via backend
  const { data: saved, isLoading: savedLoading } = useQuery({
    queryKey: ['radarr-saved'],
    queryFn: getRadarrSaved,
  })

  const [radarrUrl, setRadarrUrl] = useState('')
  const [apiKey, setApiKey] = useState('')
  const [showKey, setShowKey] = useState(false)
  const [preview, setPreview] = useState<{ total: number; movies: { tmdb_id?: number | null; title: string; year?: number | null }[] } | null>(null)
  const [connecting, setConnecting] = useState(false)
  const [previewing, setPreviewing] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [usingSaved, setUsingSaved] = useState(true)

  const savedReady = !savedLoading && saved?.configured && !saved.error
  const activeRadarrUrl = usingSaved && savedReady ? saved!.radarr_url : radarrUrl

  async function handleManualConnect() {
    if (!radarrUrl.trim() || !apiKey.trim()) return
    setConnecting(true)
    setError(null)
    setPreview(null)
    try {
      // For Radarr, we just validate by doing a preview immediately
      const data = await previewRadarr(radarrUrl.trim(), apiKey.trim())
      setPreview(data)
    } catch (e: unknown) {
      const msg = (e as { response?: { data?: { detail?: string } } })?.response?.data?.detail ?? 'Connection failed'
      setError(msg)
  ...
  async function handlePreview() {
    if (!activeRadarrUrl.trim() || (!usingSaved && !apiKey.trim())) return
    setPreviewing(true)
    setError(null)
    setPreview(null)
    try {
      const data = await previewRadarr(activeRadarrUrl.trim(), apiKey.trim())
      setPreview(data)
    } catch (e: unknown) {
      const msg = (e as { response?: { data?: { detail?: string } } })?.response?.data?.detail ?? 'Preview failed'
      setError(msg)
  ...
  async function handleImport() {
    ...
    setError(null)
    try {
      const { job_id } = await startRadarrImport(activeRadarrUrl.trim(), apiKey.trim())
      connect(job_id)
    } catch (e: unknown) {
      const msg = (e as { response?: { data?: { detail?: string } } })?.response?.data?.detail ?? 'Import failed'
      setError(msg)
  ...
  const webhookUrl = `http://${typeof window !== 'undefined' ? window.location.hostname : 'localhost'}:8899/api/radarr/webhook`
  ...
  return (
    <div className="space-y-4">
      ...
      {savedLoading && (
        <div className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-500 dark:text-slate-400">
          <Loader2 className="w-4 h-4 animate-spin" /> Checking saved Radarr credentials…
        </div>
      )}
      ...
      {!savedLoading && saved?.configured && !saved.error && (
        <div className="flex items-start justify-between gap-3 bg-green-900/20 border border-green-700 rounded-lg px-4 py-3">
          <div className="flex items-center gap-2">
            <CheckCircle className="w-4 h-4 text-green-400 flex-shrink-0" />
            <div>
              <p className="text-sm text-green-300 font-medium">Using saved Radarr credentials</p>
              <p className="text-xs text-green-600 mt-0.5">{saved.radarr_url}</p>
            </div>
          </div>
          <button
            onClick={() => { setUsingSaved(false) }}
      ...
      {(!usingSaved || (saved?.configured && saved.error)) && (
        <>
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Radarr Server URL</label>
            <input
              type="text"
              value={radarrUrl}
              onChange={(e) => { setRadarrUrl(e.target.value); setPreview(null) }}
              placeholder="http://192.168.1.100:7878"
              className="w-full px-3 py-2.5 bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:border-brand-500 transition-colors"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Radarr API Key</label>
            <div className="relative">
              <input
                type={showKey ? 'text' : 'password'}
                value={apiKey}
                onChange={(e) => { setApiKey(e.target.value); setPreview(null) }}
          ...
          <button onClick={handleManualConnect}
            disabled={!radarrUrl.trim() || !apiKey.trim() || connecting}
            className="px-4 py-2 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 disabled:opacity-50 text-white text-sm font-medium rounded-lg transition-colors flex items-center gap-2">
            {connecting ? <Loader2 className="w-4 h-4 animate-spin" /> : null}
            Connect
          </button>
      ...
      {/* Webhook info */}
      {(usingSaved && savedReady) || (preview && !error) && (
        <div className="bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-4 py-3">
          <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Webhook URL</p>
          <div className="flex items-center gap-2">
            <code className="text-xs text-slate-700 dark:text-slate-300 bg-slate-200 dark:bg-slate-700 px-2 py-1 rounded flex-1 truncate">{webhookUrl}</code>
            <button
              onClick={() => {
                navigator.clipboard.writeText(webhookUrl)
              }}
              className="text-xs px-2 py-1 rounded bg-slate-300 dark:bg-slate-700 hover:bg-slate-400 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-300"
            >
              Copy
            </button>
          </div>
          <p className="text-xs text-slate-600 dark:text-slate-500 mt-2">
            Add this URL in Radarr Settings → Connect → Webhook for automatic sync.
          </p>
        </div>
      )}

      {/* Preview and Import */}
      {(usingSaved && savedReady) || (!usingSaved && apiKey.trim() && radarrUrl.trim()) && (
        <div className="flex gap-2">
          <button onClick={handlePreview} disabled={previewing}
            className="px-4 py-2 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 disabled:opacity-50 text-white text-sm font-medium rounded-lg transition-colors flex items-center gap-2">
            {previewing ? <Loader2 className="w-4 h-4 animate-spin" /> : null}
...
type Tab = 'tmdb' | 'trakt' | 'plex' | 'radarr' | 'folder'
...
export default function Import() {
 ...
 mb-2">Bulk Import</h1>
      ...
      <div className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6">
        ...
        {activeTab === 'radarr' && <RadarrTab {...importProgress} />}
